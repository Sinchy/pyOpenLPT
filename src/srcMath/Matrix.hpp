...
// Constructor
//
template <class T> Matrix<T>::Matrix(int dim_row, int dim_col, T val) {
...
template <class T> Matrix<T>::Matrix(const Matrix<T> &mtx) {
...
template <class T> Matrix<T>::Matrix(std::string file_name) {
  std::string line;
  T value;
  ...
  if (!infile.is_open()) {
    THROW_FATAL_CTX(ErrorCode::IOfailure,
                    "Matrix<T>::Matrix error: Cannot open file: ", file_name);
  }
...
// Deconstructor
template <class T> Matrix<T>::~Matrix() { clear(); }
...
template <class T> void Matrix<T>::create(int dim_row, int dim_col) {
  // TODO: try to use nullptr for judge
  if (!_is_space) {
    _is_space = 1;
    ...
    THROW_FATAL(ErrorCode::InvalidArgument,
                "The space for _mtx is not cleared: Cannot claim more space!");
  }
}

// Get id
template <class T> inline int Matrix<T>::mapID(int id_x, int id_y) const {
  return (id_x * _dim_col + id_y);
}

// Type transform
template <class T> Matrix<double> Matrix<T>::typeToDouble() {
  Matrix<double> res(_dim_row, _dim_col, 0.0);
  for (int i = 0; i < _n; i++) {
...
template <class T> Matrix<int> Matrix<T>::typeToInt() {
  Matrix<int> res(_dim_row, _dim_col, 0.0);
  for (int i = 0; i < _n; i++) {
...
// Get/Assign value
//
template <class T> T Matrix<T>::at(int r, int c) const {
  REQUIRE_CTX(r >= 0 && r < _dim_row, ErrorCode::OutOfRange, "Invalid row",
              "row=" + std::to_string(r) + " not in [0," +
                  std::to_string(_dim_row - 1) + "]");
  REQUIRE_CTX(c >= 0 && c < _dim_col, ErrorCode::OutOfRange, "Invalid col",
              "col=" + std::to_string(c) + " not in [0," +
                  std::to_string(_dim_col - 1) + "]");
...
// Just for vector
template <class T> T Matrix<T>::operator[](int i) const { return _mtx[i]; }

// Just for vector
template <class T> T &Matrix<T>::operator[](int i) { return _mtx[i]; }

//
// Get matrix info
//
template <class T> inline int Matrix<T>::getDimRow() const { return _dim_row; }

template <class T> inline int Matrix<T>::getDimCol() const { return _dim_col; }

template <class T> void Matrix<T>::print(int precision) const {
  std::cout << std::endl;
  std::cout << "Matrix = " << std::endl;
  std::cout << "(dim_row, dim_col) = "
            << "(" << _dim_row << "," << _dim_col << ")" << std::endl;
  int id_start;
  for (int iter_x = 0; iter_x < _dim_row; iter_x++) {
    ...
    for (int iter_y = 0; iter_y < _dim_col; iter_y++) {
      std::cout << std::scientific << std::setprecision(precision)
                << std::setfill(' ') << std::left << _mtx[id_start + iter_y]
                << " ";
    }
    std::cout << "|" << std::endl;
  }
...
template <class T> const T *Matrix<T>::data() const { return _mtx; }

template <class T> void Matrix<T>::setData(const T *data, int size) {
  REQUIRE(size == _n, ErrorCode::InvalidArgument,
          "Matrix<T>::setData error: The size of input data does not match the "
          "size of matrix!");
...
template <class T> double Matrix<T>::norm() {
  double res = 0;
  for (int i = 0; i < _n; i++) {
    res += (_mtx[i] * _mtx[i]);
  }
...
// Matrix output
//
template <class T> void Matrix<T>::write(std::string file_name) {
...
template <class T> void Matrix<T>::write(std::ostream &os) {
...
template <class T> Matrix<T> &Matrix<T>::operator=(Matrix<T> const &mtx) {
...
template <class T> bool Matrix<T>::operator==(Matrix<T> const &mtx1) {
...
template <class T> Matrix<T> Matrix<T>::operator+(Matrix<T> const &mtx) const {
  if ((_dim_row != mtx._dim_row) || (_dim_col != mtx._dim_col)) {
    THROW_FATAL(ErrorCode::InvalidArgument,
                "The size of matrices do not match!");
  }
...
template <class T> Matrix<T> Matrix<T>::operator+(T delta) const {
  Matrix<T> res(*this);
  for (int i = 0; i < _n; i++) {
    res._mtx[i] += delta;
  }
...
template <class T> Matrix<T> &Matrix<T>::operator+=(T delta) {
  for (int i = 0; i < _n; i++) {
    _mtx[i] += delta;
  }
  return *this;
...
template <class T> Matrix<T> Matrix<T>::operator*(T ratio) const {
  Matrix<T> res(*this);
  for (int i = 0; i < _n; i++) {
    res._mtx[i] *= ratio;
  }
...
template <class T> Matrix<T> &Matrix<T>::operator*=(T ratio) {
  for (int i = 0; i < _n; i++) {
    _mtx[i] *= ratio;
  }
  return *this;
...
// Get Row
template <class T> std::vector<T> Matrix<T>::getRow(int row_i) const {
...
// Get Col
template <class T> std::vector<T> Matrix<T>::getCol(int col_j) const {
